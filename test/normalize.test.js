...
const TS = String(Math.floor(NOW.getTime() / 1000)); // fact timestamp for "today"
const todayKey = dateKey(kyivDateParts(NOW));
const onToday = (intervals) => intervals.filter((i) => i.start.startsWith(todayKey));

test('preset hour codes become merged intervals; group label and name are derived', () => {
  const raw = {
    preset: {
      data: { 'GPV1.1': everyWeekday({ 9: 'no', 10: 'no', 11: 'maybe', 12: 'first' }) },
      sch_names: { 'GPV1.1': 'Черга 1.1' },
    },
    fact: { data: {} },
  };
...
test('fact overrides preset for the dates it covers', () => {
  const raw = {
    preset: { data: { 'GPV1.1': everyWeekday({ 9: 'no' }) } },
    fact: { data: { [TS]: { 'GPV1.1': { 20: 'no', 21: 'no' } } } },
  };
...
test('groups are discovered from both preset and fact and sorted', () => {
  const raw = {
    preset: { data: { 'GPV2.1': everyWeekday({ 1: 'no' }), 'GPV1.2': everyWeekday({ 1: 'no' }) } },
    fact: { data: { [TS]: { 'GPV1.1': { 1: 'no' } } } },
  };
...
test('unknown / "yes" codes produce no interval', () => {
  const raw = { preset: { data: { 'GPV1.1': everyWeekday({ 9: 'yes', 10: 'bogus' }) } }, fact: { data: {} } };
  const { groups, schedules } = normalize(raw, NOW);
  assert.deepEqual(groups, ['1.1']);
...
test('fixture sample parses end-to-end', async () => {
  ...
  assert.equal(schedules['2.1'].name, 'Черга 2.1');
});
