...
- `data/log.jsonl` — історія запусків, по одному JSON-об'єкту на рядок
  (найновіші — в кінці, зберігаються останні 1000 записів). Записуються запуски
  зі змінами чи збоями, а також один «тихий» запуск щогодини (heartbeat);
...
зміну, а не час опитування. Так само `log.jsonl` фіксує запуск, якщо той має
що сказати (змінились дані або стався збій), а «тихі» запуски — раз на годину
як heartbeat. Тому гілка `data` отримує ~30 комітів на добу, а не по одному на
кожне з ~270 опитувань:
